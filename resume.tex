...
\usepackage[a4paper,margin=\margin]{geometry} %a4paper sets margins, can add showframe to display guidelines
...
%\setlength{\columnwidth}{0.75\paperwidth-0.4\margin}
...
%\parskip=0.75em
\begin{tikzpicture}[overlay, xshift = -\margin, yshift=-\headerheight + \margin*2 ]

\coordinate (dm1) at (0,0);
\coordinate (dm2) at (\paperwidth,\headerheight);
\node[rectangle,fill=light-gray] [fit=(dm1) (dm2)] (headerbx) {};
\node[align=center,font=\large] at (headerbx.center) {
    \begin{minipage}{\linewidth}
        \vskip \margin
        \begin{vwcol}[widths={0.7,0.3},justify=flush,sep=1cm,rule=1pt,indent=0em] 
            %\lipsum[1]
            {\LARGE \textbf{NOAH E WOLFE}} \\[0.5em]
            {\small ASTROPHYSICS | ENVIRONMENTAL JUSTICE} \\
            \textbf{T} \ \ \  704 998 1322 \\
            \textbf{1} \ \ \ newolfe@ncsu.edu \\
            \textbf{2} \ \ \ noah@scivir.org
        \end{vwcol}
    \end{minipage}
